    ...
    with open(pathmapFileName, 'r') as pathmap_handle:
        for parm, path in zip(parms, pathmap_handle.readlines()):
            ...
            if not raw_value == path.strip():
                try:
                    print('Updating Parm "%s" from "%s" to "%s"' %
                          (parm.path(), raw_value, path.strip()))
                    parm.set(path.strip())
                except hou.OperationFailed as e:
                    print('Failed to update Parm "%s" received the following exception: %s' %
                          (parm.path(), e))
        ...
        if ropType == 'rop_geometry':
            ...
            if output is not None:
                ...
                if total_displays == 1:
                    ...
                    raise RuntimeError(
                        "Override Output is supported only for a single display but found {0}.".format(total_displays))
            ...
            if output is not None:
                ...
                if not os.path.isdir(output_folder):
                    try:
                        ...
                        print(
                            'Failed to create output directory "%s". The path may be invalid or permissions may not be sufficient.' % output_folder)
                        raise
            ...
            if gpus is not None:
                print(
                    "This Slave is overriding its GPU affinity, so the following GPUs will be used by RedShift: " + gpus)
                gpus = gpus.split(",")
                gpuSettingString = ""
                for i in range(8):
                    if str(i) in gpus:
            ...
            if gpus is not None:
                print(
                    "This Slave is overriding its GPU affinity, so the following GPUs will be used by Octane: " + gpus)
                gpus = gpus.split(",")
                for i in range(16):
