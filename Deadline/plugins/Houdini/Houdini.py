...
class HoudiniPlugin (DeadlinePlugin):
    ...
    def InitializeProcess(self):
        ...
        self.AddStdoutHandlerCallback(
            "(Couldn't find renderer.*)").HandleCallback += self.HandleStdoutRenderer
        self.AddStdoutHandlerCallback(
            "(Error: Unknown option:.*)").HandleCallback += self.HandleStdoutUnknown
        self.AddStdoutHandlerCallback("(Error: .*)").HandleCallback += self.HandleStdoutError
        self.AddStdoutHandlerCallback(
            r"(ERROR\s*\|.*)").HandleCallback += self.HandleStdoutError  # Arnold errors
        self.AddStdoutHandlerCallback(r"\[Error\].*").HandleCallback += self.HandleStdoutError
        self.AddStdoutHandlerCallback(
            ".*(Redshift cannot operate with less than 128MB of free VRAM).*").HandleCallback += self.HandleStdoutError
        self.AddStdoutHandlerCallback(
            ".*(No licenses could be found to run this application).*").HandleCallback += self.HandleStdoutLicense
        self.AddStdoutHandlerCallback(
            ".*ALF_PROGRESS ([0-9]+)%.*").HandleCallback += self.HandleStdoutFrameProgress
        self.AddStdoutHandlerCallback(
            ".*Render Time:.*").HandleCallback += self.HandleStdoutFrameComplete
        self.AddStdoutHandlerCallback(
            ".*Finished Rendering.*").HandleCallback += self.HandleStdoutDoneRender
        self.AddStdoutHandlerCallback(".*ROP type: (.*)").HandleCallback += self.SetRopType
        self.AddStdoutHandlerCallback(
            ".*?(\d+)% done.*").HandleCallback += self.HandleStdoutFrameProgress
        self.AddStdoutHandlerCallback(
            "\[render progress\] ---[ ]+(\d+) percent").HandleCallback += self.HandleStdoutFrameProgress
        self.AddStdoutHandlerCallback(
            "(License error: No license found)").HandleCallback += self.HandleStdoutLicense
        self.AddStdoutHandlerCallback(
            "RMAN_PROGRESS *([0-9]+)%").HandleCallback += self.HandleStdoutFrameProgress

        # redshift errors if output path does not exist
        self.AddStdoutHandlerCallback(
            "Output file locked or path not available for writing. Skipping frame.").HandleCallback += self.HandleStdoutOutputPathUnavailable
    ...
    def RenderExecutable(self):
        ...
        if SystemUtils.IsRunningOnWindows():
            if build == "32bit":
                ...
                if houdiniExe == "":
                    self.LogWarning("Houdini " + version + " hython 32 bit executable was not found in the semicolon separated list \"" +
                                    houdiniExeList + "\". Checking for any executable that exists instead.")
            elif build == "64bit":
                self.LogInfo("Enforcing 64 bit build")
                houdiniExe = FileUtils.SearchFileListFor64Bit(houdiniExeList)
                if houdiniExe == "":
                    self.LogWarning("Houdini " + version + " hython 64 bit executable was not found in the semicolon separated list \"" +
                                    houdiniExeList + "\". Checking for any executable that exists instead.")

        if houdiniExe == "":
            houdiniExe = FileUtils.SearchFileList(houdiniExeList)
            if houdiniExe == "":
                self.FailRender("Houdini " + version + " hython executable was not found in the semicolon separated list \"" + houdiniExeList +
    ...
    def RenderArgument(self):
        ...
        if simJob:
            ...
            if self.GetBooleanPluginInfoEntryWithDefault("SimRequiresTracking", True):
                ...
                if tasks.GetTask(0).TaskStatus != "Rendering":
                    self.FailRender(
                        "Sim Job: Cannot determine which machine is running the tracking process because the first task for this job is not in the rendering state")

                trackerMachineSlave = tasks.GetTask(0).TaskSlaveName
                if trackerMachineSlave == "":
                    self.FailRender(
                        "Sim Job: Cannot determine which machine is running the tracking process because the first task for this job is not being rendered by another Worker")

                slaveInfo = RepositoryUtils.GetSlaveInfo(trackerMachineSlave, True)
                self.LogInfo("Sim Job: Worker \"" + slaveInfo.SlaveName +
                             "\" is running the tracking proccess")
                ...
                if not self.GetConfigEntry("Houdini_SimTracker_Use_IP_Address"):
                    ...
                    self.LogInfo(
                        "Sim Job: Connecting to Worker machine using host name \"" + machineNameOrIpAddress + "\"")
                else:
                    machineNameOrIpAddress = SlaveUtils.GetMachineIPAddresses([slaveInfo])[0]
                    self.LogInfo(
                        "Sim Job: Connecting to Worker machine using IP address \"" + machineNameOrIpAddress + "\"")

            arguments.append("-s %s \"%s\" %s" %
                             (singleRegionFrame, machineNameOrIpAddress, trackerPort))
        ...
        if regionRendering:
            ...
            if singleRegionJob:
                ...
                xStart = self.GetFloatPluginInfoEntryWithDefault(
                    "RegionLeft" + str(singleRegionIndex), 0)
                xEnd = self.GetFloatPluginInfoEntryWithDefault(
                    "RegionRight" + str(singleRegionIndex), 0)
                yStart = self.GetFloatPluginInfoEntryWithDefault(
                    "RegionBottom" + str(singleRegionIndex), 0)
                yEnd = self.GetFloatPluginInfoEntryWithDefault(
                    "RegionTop" + str(singleRegionIndex), 0)
            else:
        ...
        if len(gpuList) > 0:
            ...
            if self.GetBooleanPluginInfoEntryWithDefault("OpenCLUseGPU", 1):
                ...
                self.SetEnvironmentVariable("HOUDINI_OCL_DEVICENUMBER",
                                            gpuList[self.GetThreadNumber() % len(gpuList)])

        tempDir = self.CreateTempDirectory(str(self.GetThreadNumber()))
        arguments.append("-tempdir \"%s\"" % tempDir)

        abortOnLicenseFail = 1 if self.GetBooleanConfigEntryWithDefault(
            "AbortOnArnoldLicenseFail", True) else 0
        ...
        if self.OverrideGpuAffinity():
            ...
            if gpusPerTask == 0 and gpusSelectDevices != "":
                ...
                if len(notFoundGPUs) > 0:
                    self.LogWarning(
                        "The Worker is overriding its GPU affinity and the following GPUs do not match the Workers affinity so they will not be used: " + ",".join(notFoundGPUs))
                if len(resultGPUs) == 0:
                    self.FailRender(
                        "The Worker does not have affinity for any of the GPUs specified in the job.")
            elif gpusPerTask > 0:
                if gpusPerTask > len(overrideGPUs):
                    self.LogWarning("The Worker is overriding its GPU affinity and the Worker only has affinity for " + str(
                        len(overrideGPUs)) + " gpus of the " + str(gpusPerTask) + " requested.")
    ...
    def PreRenderTasks(self):
        ...
        if self.GetBooleanPluginInfoEntryWithDefault("SimJob", False) and self.GetBooleanPluginInfoEntryWithDefault("SimRequiresTracking", True) and self.GetCurrentTaskId() == "0":
            self.LogInfo(
                "Sim Job: Starting Sim Tracker process because this is the first task for this sim job")
            ...
            if simTracker == "":
                self.FailRender("Sim Job: Houdini " + version + " sim tracker file was not found in the semicolon separated list \"" +
                                simTrackerList + "\". The path to the sim tracker file can be configured from the Plugin Configuration in the Deadline Monitor.")
            ...
            if trackerPortInUse and webServicePortInUse:
                self.FailRender("Unable to start the Sim Tracker process because tracker port {0} and web service port {1} are in use.".format(
                    trackerPort, webServicePort))
            elif trackerPortInUse:
                self.FailRender(
                    "Unable to start the Sim Tracker process because tracker port {0} is in use.".format(trackerPort))
            elif webServicePortInUse:
                self.FailRender(
                    "Unable to start the Sim Tracker process because web service port {0} is in use.".format(webServicePort))

            arguments = "\"" + simTracker + "\" " + str(trackerPort) + " " + str(webServicePort)
            self.LogInfo("Sim Job: Starting the Sim Tracker process")
            # string name, string executable, string arguments, string startupDirectory
            self.StartMonitoredProgram("SimTracker", pythonPath, arguments,
                                       Path.GetDirectoryName(simTracker))
    ...
    def PostRenderTasks(self):
        if self.GetBooleanPluginInfoEntryWithDefault("SimJob", False) and self.GetBooleanPluginInfoEntryWithDefault("SimRequiresTracking", True) and self.GetCurrentTaskId() == "0":
            self.LogInfo(
                "Sim Job: Waiting for all other tasks for this job to complete before stopping theSim Tracker process")
    ...
    def HandleStdoutFrameProgress(self):
        if self.ropType in ("ifd", "rop_ifd"):
            ...
            if frameCount != 0:
                ...
                overallProgress = (completedFrameProgress +
                                   currentFrameProgress) / float(frameCount)
    ...
    def SetRedshiftPathmappingEnv(self, mappings):
        try:
            ...
            newRSMappingFileName = os.path.join(
                self.CreateTempDirectory("RSMapping"), "RSMapping.txt")
