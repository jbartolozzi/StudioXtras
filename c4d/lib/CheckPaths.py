...
def updateFilePaths(assets_to_check, pathmapper):
    num_missing = 0
    num_updated = 0
    for asset in assets_to_check:
        updated = pathmapper.updatePath(asset.getFilePath())
        if not os.path.exists(updated):
            num_missing += 1
            print("CheckPaths Updated Path \"%s\" doesnt exist for node \"%s\"." % (updated, asset.getName()))
    ...
    for mat in list(mat for mat in doc.GetMaterials() if mat.GetType() == CINEMA_SHADER_NETWORK):
        ...
        if mat[c4d.MATERIAL_USE_REFLECTION]:
            for layernum in range(mat.GetReflectionLayerCount()):
                ...
                new_object = CinemaTexture(mat[layer.GetDataID() + c4d.REFLECTION_LAYER_COLOR_TEXTURE])
                if new_object:
                    output.append(new_object)
...
def getAllArnoldProcedurals(doc, all_objects):
    output = []
    arnold_procedurals = list(obj for obj in all_objects if obj.GetTypeName() == "Arnold Procedural")
...
def main(doc):
    ...
    if len(all_assets):
        (num_img_updated, num_img_missing) = updateFilePaths(all_assets, pathmapper)
